...
export function usePendingInvoices() {
  ...
  const load = useCallback((params?: any) => dispatch(listPendingThunk(params)), [dispatch]);
...
export function useApprovedInvoices() {
  ...
  const load = useCallback((params?: any) => dispatch(listApprovedThunk(params)), [dispatch]);
  return { items, status, load };
}
...
export function useInvoiceApproval() {
  ...
  const approveOne = useCallback((id: number, remarks?: string) => dispatch(approveInvoiceThunk({ id, remarks })), [dispatch]);
  const rejectOne = useCallback((id: number, remarks: string) => dispatch(rejectInvoiceThunk({ id, remarks })), [dispatch]);
