...
export function validateGstNumber(value) {
    if (!value || value.trim() === '')
        return undefined; // Make GST number optional
    const v = value.trim();
    if (!GST_REGEX.test(v))
...
export function validateRequiredString(value, label = 'Field') {
    if (!value || !value.toString().trim())
        return `${label} is required`;
    return undefined;
}
export function validateNumber(value, label = 'Field') {
    if (value === undefined || value === null || value === '')
        return undefined; // Make financial fields optional
    const num = Number(value);
    if (Number.isNaN(num))
        return `${label} must be a number`;
    return undefined;
}
export function validateNonNegative(value, label = 'Field') {
...
export function validateOptionalNonNegative(value, label = 'Field') {
    if (value === undefined || value === null || value === '')
        return undefined; // Make optional
    const err = validateNumber(value, label);
    if (err)
        return err;
    if (Number(value) < 0)
